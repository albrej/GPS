...
def exporter_vers_gpx(points, chemin_sortie, garder_temps=True, waypoints=None):
    ...
    # Annotations (photos/vidéos prises pendant un suivi live, voir
    # LiveScreen._ouvrir_camera_Android) : ajoutées comme waypoints GPX,
    # visibles comme repères dans n'importe quel logiciel GPS.
    if waypoints:
        for w in waypoints:
...
def calculer_vitesse_kmh_entre_points(p_precedent, p_suivant):
    """Calcule la vitesse instantanee (km/h) entre deux points GPS
    successifs, a partir de leur horodatage. Retourne None si le calcul
    est impossible."""
    if not p_precedent or not p_suivant:
        return None
    if not p_precedent.get('time') or not p_suivant.get('time'):
        return None
    dt = (p_suivant['time'] - p_precedent['time']).total_seconds()
    if dt <= 0:
        return None
    d = calculer_distance_haversine(p_precedent['lat'], p_precedent['lon'], p_suivant['lat'], p_suivant['lon'])
